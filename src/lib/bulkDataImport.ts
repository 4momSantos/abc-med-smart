...
export async function bulkInsertMedicines(items: MedicineItem[]): Promise<{
  ...
  try {
    ...
    // Preparar dados para inserção (estrutura híbrida: campos principais + extra_data JSONB)
    const medicinesData = items.map(item => ({
      // Campos principais (indexáveis)
      code: item.code,
      name: item.name,
      ...
      // Campos extras em JSONB (economia de memória)
      extra_data: {
        unit: item.unit,
